...
calculate events = guardId * minute
  where (guardId, minutes) = sleepiestGuard . sleepPerGuard $ sleepPerShift events
        (_, minute) = sleepiestMinute minutes
...
sleepiestGuard = maximumBy (comparing (fst . sleepiestMinute . snd)) . M.toList

sleepiestMinute :: [Int] -> (Int, Int)
sleepiestMinute [] = (0, 0)
sleepiestMinute i = last . sort . map (\l@(e:_) -> (length l, e)) . group . sort $ i
